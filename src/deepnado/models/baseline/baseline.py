...
from deepnado.models.baseline.coordinates import CoordConv2D
from deepnado.common.constants import CHANNEL_MIN_MAX, ALL_VARIABLES


class TornadoClassifier(LightningModule):
    """
    ...
    def training_step(self, batch, _):
        ...
        logits = max_pool2d(logits, kernel_size=logits.size()[2:])  # [batch,1,1,1]
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            "scheduler": StepLR(optimizer, step_size=self.lr_decay_steps, gamma=self.lr_decay_rate),
...
class TornadoLikelihood(nn.Module):
    ...
    def __init__(
        ...
        self.blk1 = VggBlock(
            in_channels, in_coords, start_filters, kernel_size=3, n_convs=2, drop_rate=0.1
        )  # (60,120)
        self.blk2 = VggBlock(
            start_filters, in_coords, 2 * start_filters, kernel_size=3, n_convs=2, drop_rate=0.1
        )  # (30,60)
        self.blk3 = VggBlock(
            2 * start_filters, in_coords, 4 * start_filters, kernel_size=3, n_convs=3, drop_rate=0.1
        )  # (15,30)
        self.blk4 = VggBlock(
            4 * start_filters, in_coords, 8 * start_filters, kernel_size=3, n_convs=3, drop_rate=0.1
        )  # (7,15)
    ...
    def forward(self, data: Dict[str, Any]):
        ...
        #  [batch,tilt*len(input_variables)*2,Az,Rng]
        x = torch.cat([x[v] for v in self.input_variables], axis=1)
